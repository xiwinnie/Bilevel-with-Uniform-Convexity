...
class Learner(nn.Module):
    ...
    def forward(self, train_loader, val_loader, training=True, epoch=0):
        ...
        for step, train_batch in enumerate(train_loader):
            ...
            self.neumann_series(self.args, q_loss, next(iter(val_loader)), train_batch)
    ...
    def neumann_series(self, args, loss, query_batch, support_batch):
        ...
        F_gradient = [(g_param/((args.p_norm-1) *param.detach().pow(args.p_norm-2))).view(-1) for (g_param, param) in zip(Fy_gradient, self.inner_model.parameters())] # grad 
        ...
        for g_grad, param in zip(Gy_gradient, self.inner_model.parameters()):
            G_gradient.append((param - args.neumann_lr * g_grad/((args.p_norm - 1)*param.detach().pow(args.p_norm-2))).view(-1))
        ...
        self.hyper_momentum = [args.beta * h + (1 - args.beta) *  Gyxz for (h, Gyxz) in
                          zip(self.hyper_momentum,  Gyxv_gradient)]
        # return outer_update
